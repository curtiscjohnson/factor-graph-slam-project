...
def compose(r1, r1to2):
    th1 = r1[2]
    th12 = -r1to2[2]
    A1 = np.array([[np.cos(th1), -np.sin(th1), r1[0]],
                   [np.sin(th1), np.cos(th1), r1[1]],
                   [0, 0, 1]])

    A12 = np.array([[np.cos(th12), -np.sin(th12), r1to2[0]],
                   [np.sin(th12), np.cos(th12), r1to2[1]],
                   [0, 0, 1]])

    A2 = A1 @ A12

    r2 = A2[0:3, 2]
    r2[2] = np.arctan2(A2[1, 0], A2[0, 0])
    return r2